...
namespace ghost
{
	...
	{
		/*!
		 * Implementation of the Linear Equation constraint with greater than, i.e., the Scalar Product constraint with the atom '>'.
		 * See http://sofdem.github.io/gccat/gccat/Cscalar_product.html
		 */
		class LinearEquationG : public LinearEquation
		{
		...
		public:
			 ...
			 */
			LinearEquationG( const std::vector<int>& variables_index, double rhs, const std::vector<double>& coefficients );

			/*!
			 * Constructor with a vector of variable IDs. This constructor calls LinearEquationG( const std::vector<int>& index, double rhs, const std::vector<double>& coefficients), with all coefficients set to 1.0.
			 * \param variables_index a const reference to a vector of IDs of variables composing the constraint.
			 * \param rhs the right-hand side real number of the equation.
			 */
			LinearEquationG( const std::vector<int>& variables_index, double rhs );
			 ...
			 * \param coefficients the vector of real numbers coefficients for each variable of the equation.
			 */
			LinearEquationG( const std::vector<Variable>& variables, double rhs, const std::vector<double>& coefficients );

			/*!
			 * Constructor with a vector of variable. This constructor calls LinearEquationG( const std::vector<Variable>& variables, double rhs, const std::vector<double>& coefficients), with all coefficients set to 1.0.
			 * \param variables a const reference to a vector of variables composing the constraint.
			 * \param rhs the right-hand side real number of the equation.
			 */
			LinearEquationG( const std::vector<Variable>& variables, double rhs );
};
	}
}
