...
LinearEquationL::LinearEquationL( const std::vector<int>& variables_index, double rhs )
	: LinearEquationL( variables_index, rhs, std::vector<double>( variables_index.size(), 1.0 ) )
{ }
...
LinearEquationL::LinearEquationL( const std::vector<Variable>& variables, double rhs )
	: LinearEquationL( variables, rhs, std::vector<double>( variables.size(), 1.0 ) )
{ }

double LinearEquationL::required_error( const std::vector<Variable*>& variables ) const
{
	_current_sum = 0.0;
	...
	return std::max( 0.0, _current_sum - _rhs ) + equals;
}

double LinearEquationL::optional_delta_error( const std::vector<Variable*>& variables,
                                             ...
                                             const std::vector<int>& candidate_values ) const
{
	...
	return std::max( 0.0, sum - _rhs ) + equals_sum - std::max( 0.0, _current_sum - _rhs ) - equals_current;
} 

void LinearEquationL::conditional_update_data_structures( const std::vector<Variable*>& variables, int variable_index, int new_value ) 
{
