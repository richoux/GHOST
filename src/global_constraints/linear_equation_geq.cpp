...
LinearEquationGeq::LinearEquationGeq( const std::vector<int>& variables_index, double rhs )
	: LinearEquationGeq( variables_index, rhs, std::vector<double>( variables_index.size(), 1.0 ) )
{ }
...
LinearEquationGeq::LinearEquationGeq( const std::vector<Variable>& variables, double rhs )
	: LinearEquationGeq( variables, rhs, std::vector<double>( variables.size(), 1.0 ) )
{ }

double LinearEquationGeq::required_error( const std::vector<Variable*>& variables ) const
{
	_current_sum = 0.0;
	for( size_t i = 0 ; i < variables.size() ; ++i )
		_current_sum += ( _coefficients[i] * variables[i]->get_value() );

	return std::max( 0.0, _rhs - _current_sum );
}

double LinearEquationGeq::optional_delta_error( const std::vector<Variable*>& variables,
                                                ...
                                                const std::vector<int>& candidate_values ) const
{
	...
	return std::max( 0.0, _rhs - sum ) - std::max( 0.0, _rhs - _current_sum );
} 

void LinearEquationGeq::conditional_update_data_structures( const std::vector<Variable*>& variables, int variable_index, int new_value ) 
{
